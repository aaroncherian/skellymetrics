...
class ErrorMetricsBuilder:
    ...
    def calculate_squared_error_dataframe(self, dataframe_of_3d_data):
        ...
        The DataFrame should contain columns ['frame', 'marker', 'x', 'y', 'z', 'x_velocity, 'y_velocity', 'z_velocity',
        'system']
        ...
        for coord in ['x', 'y', 'z', 'x_velocity', 'y_velocity', 'z_velocity']:
            ...
            squared_error_dataframe[f'{coord}_error'] = self._calculate_squared_error(pivot_df[coord][system_A_name],
                                                                                      pivot_df[coord][
                                                                                          system_B_name]).reset_index(
                drop=True)  # need to reset index here to make sure there's not a dataframe index mismatch
    ...
    def calculate_absolute_error_from_squared_error_dataframe(self):
        ...
        if self.squared_error_dataframe is None:
            raise ValueError(
                'Squared error dataframe has not been calculated yet. Please run calculate_squared_error_dataframe() first.')
        ...
        for coord in ['x', 'y', 'z', 'x_velocity', 'y_velocity', 'z_velocity']:
            absolute_error_dataframe[f'{coord}_error'] = self._calculate_absolute_error_from_squared_error(
                self.squared_error_dataframe[f'{coord}_error'])
    ...
    def calculate_rmse_from_dataframe(self):
        ...
        rmse_joints = self.squared_error_dataframe.groupby('marker', sort=False)[
            ['x_error', 'y_error', 'z_error', 'x_velocity_error', 'y_velocity_error', 'z_velocity_error']].apply(
            self._calculate_rmse_from_squared_error).reset_index()
        ...
        rmse_dimensions = self.squared_error_dataframe[
            ['x_error', 'y_error', 'z_error', 'x_velocity_error', 'y_velocity_error', 'z_velocity_error']].apply(
            self._calculate_rmse_from_squared_error).reset_index()
